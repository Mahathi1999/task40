...
// API endpoint to get all customers with booked data
app.get("/customers", (req, res) => {
  const customersWithBookings = bookings.map((booking) => {
    const room = rooms.find((room) => room.roomName === booking.roomName);
    return {
  ...
  return res.status(200).json(customersWithBookings);
});
